...
with tab2:
    ...
    with col1:
        sim_speed = st.slider(
            f"💨 {t('wind_speed_label', lang)}",
            min_value=0, max_value=120,
            value=25, step=1, key="sim_speed"
        )
    with col2:
        sim_direction = st.slider(
            f"🧭 {t('direction_label', lang)}",
            min_value=0, max_value=359,
            value=300, step=1, key="sim_dir"
        )
    with col3:
        sim_hours = st.slider(
            f"⏱️ {t('hours_ahead', lang)}",
            min_value=1, max_value=12,
            value=3, step=1, key="sim_hours"
        )

    travel_km = round(sim_speed * sim_hours)

    st.markdown(f"""
    <div style='background:linear-gradient(135deg,rgba(0,180,216,0.1),rgba(0,119,182,0.1));
                border:1px solid rgba(0,180,216,0.3); border-radius:12px;
                padding:1rem 1.5rem; margin:1rem 0;
                display:flex; align-items:center; gap:12px; flex-wrap:wrap;'>
        <span style='font-size:1.5rem;'>💨</span>
        <span style='color:white; font-size:0.95rem;'>
            <b style='color:#00b4d8;'>{sim_speed} km/h</b>
            {t('from', lang)}
            <b style='color:#a855f7;'>{sim_direction}° ({compass(sim_direction)})</b>
            —
            {t('smoke_travels', lang)}
            <b style='color:#f39c12;'>{travel_km} km</b>
            {t('in', lang)}
            <b style='color:#2ecc71;'>{sim_hours}h</b>
        </span>
    </div>
    """, unsafe_allow_html=True)

    sim_results = calculate_risk(sim_speed, sim_direction, sim_hours)
    sim_alerted = set(a["factory"] for a in sim_results)
    sim_people  = sum(
        CITY_POPULATIONS.get(a["target_zone"], 0) for a in sim_results
    )

    s1, s2, s3 = st.columns(3)
    with s1: metric_card(t("people_at_risk", lang), f"{sim_people:,}", "#e74c3c")
    with s2: metric_card(t("factories_risk", lang), len(sim_alerted), "#f39c12")
    with s3: metric_card(t("safe_factories", lang), len(FACTORIES) - len(sim_alerted), "#2ecc71")

    st.divider()

    if sim_speed == 0:
        st.success("✅ Wind speed is zero — no smoke movement.")
    elif sim_results:
        st.error(f"⚠️ {len(sim_results)} {t('alert_banner', lang)}")
    else:
        st.success(f"✅ {t('all_clear', lang)}")

    ml, mr = st.columns([3, 2])

    with ml:
        section_header("🗺️", t("simulation_map", lang))
        build_map(sim_results, sim_hours, key="sim_map")

    with mr:
        section_header("📊", t("results", lang))
        if sim_results:
            for a in sim_results:
                risk  = a["risk_score"]
                color = "#e74c3c" if risk >= 70 else "#f39c12" if risk >= 40 else "#f1c40f"
                icon  = "🔴" if risk >= 70 else "🟠" if risk >= 40 else "🟡"
                st.markdown(f"""
                <div style='background:linear-gradient(135deg,rgba(30,37,53,0.9),rgba(37,45,66,0.9));
                            border-left:4px solid {color}; border-radius:10px;
                            padding:0.8rem 1rem; margin-bottom:0.5rem;'>
                    <div style='color:{color}; font-weight:800;'>{icon} {a['factory']}</div>
                    <div style='color:rgba(255,255,255,0.6); font-size:0.85rem;'>
                        → {a['target_zone']}
                    </div>
                    <div style='color:white; font-size:0.9rem; margin-top:4px;'>
                        Risk: <b style='color:{color};'>{risk}/100</b>
                        &nbsp;|&nbsp; {a['distance_km']} km
                    </div>
                </div>
                """, unsafe_allow_html=True)
        else:
            st.success(t("no_risk", lang))

        st.divider()
        section_header("📋", t("scenario_summary", lang))
        data = {
            t("wind_speed", lang):       f"{sim_speed} km/h",
            "Direction":                 f"{sim_direction}° ({compass(sim_direction)})",
            t("hours_ahead", lang):      f"{sim_hours}h",
            t("travel_distance", lang):  f"{travel_km} km",
            t("factories_checked", lang):len(FACTORIES),
            t("zones_at_risk", lang):    len(sim_results),
            t("people_at_risk", lang):   f"{sim_people:,}",
        }
        for k, v in data.items():
            st.markdown(f"""
            <div style='display:flex; justify-content:space-between;
                        padding:6px 0; border-bottom:1px solid rgba(255,255,255,0.05);'>
                <span style='color:rgba(255,255,255,0.5); font-size:0.85rem;'>{k}</span>
                <span style='color:#00b4d8; font-weight:700; font-size:0.85rem;'>{v}</span>
            </div>
            """, unsafe_allow_html=True)
